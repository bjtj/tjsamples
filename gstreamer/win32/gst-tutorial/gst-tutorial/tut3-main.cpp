...
static int tut3(int argc, char* argv[]);
static int tut3_exercise(int argc, char* argv[]);

int main(int argc, char* argv[]) {
    tut3(argc, argv);
    tut3_exercise(argc, argv);
}
...
static int tut3_exercise(int argc, char* argv[]) {
    ...
    if (!data.pipeline || !data.source || !data.videoconvert || !data.videosink || !data.convert || !data.resample || !data.sink) {
        g_printerr("Not all elements could be created.\n");
        return -1;
    }
    ...
    /* Connect to the pad-added signal */
    g_signal_connect(data.source, "pad-added", G_CALLBACK(pad_added_handler), &data);
    g_signal_connect(data.source, "pad-added", G_CALLBACK(pad_added_video_handler), &data);

...
public sealed class TimerService: IHostedService, IAsyncDisposable
{
  ...
  private void DoWork(object? state)
  {
    ...
    _logger.LogInformation("{Service} is working, execution count: {Count:#,0}", nameof(TimerHostedService), count);
  }

  public Task StopAsync(CancellationToken stoppingToken)
  {
    _logger.LogInformation("{Service} is stopping.", nameof(TimerHostedService));
